...
//            'post_content' => $data['content'],
        ...
        if($data['whats_new']) {
                $row = array(
                'extra_info_title' => 'whats_new',
                'extra_info_desc' => $data['whats_new']
            );
            add_row('extra_info', $row, $post_id);
        }

        if ( ! empty($data['category']) ) {
            $terms = array();
            if ( $term_id = term_exists($data['category'], 'category') ) {
                $term = $term_id;
            } else {
        ...
        if ( !empty($data['publisher']) ) {
            $terms = array();
            if ( $term_id = term_exists($data['publisher'], 'publisher') ) {
                $term = $term_id;
            } else {
                $term = wp_insert_term($data['publisher'], 'publisher' );
            }
            $terms[] = $term['term_id'];

            wp_set_post_terms( $post_id, $terms, 'publisher' );
        }
