...
add_action( 'init', 'app_init_tip' );

// Add tip columns
function app_tip_columns_head($cols) {		
	$cols = array_merge(
...
function app_tip_columns_content($col_name, $post_ID) {
...
add_filter('manage_tip_posts_columns', 'app_tip_columns_head');
add_action('manage_tip_posts_custom_column', 'app_tip_columns_content', 20, 2);
 ...
 */
function app_rewrite_url_tip_cat_taxonomy($wp_rewrite) {
...
add_filter('generate_rewrite_rules', 'app_rewrite_url_tip_cat_taxonomy');

function app_tip_cat_link( $link, $term, $taxonomy ) {
    if ( $taxonomy !== 'tip_cat' )
        return $link;

    return str_replace( '/tip_cat/', '/', $link );
}
add_filter( 'term_link', 'app_tip_cat_link', 10, 3 );