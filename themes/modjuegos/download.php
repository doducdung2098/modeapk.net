...
$hide_ads = get_field('hide_ads', $post_id);
...
if ($versions && $affs) {
    foreach ($versions as $version_key => $version) {
        foreach ($version['version_downloads'] as $key => $value) {
            foreach ($affs as $aff) {
                if (strpos($value['version_download_link'], $aff['aff_host']) !== false) {
                    if (parse_url($value['version_download_link'], PHP_URL_QUERY))
                        $versions[$version_key][$key]['version_download_link'] = $value['version_download_link'] . '&' . $aff['aff_code'];
                    else
                        $versions[$version_key][$key]['version_download_link'] = $value['version_download_link'] . '?' . $aff['aff_code'];
                }
            }
        ...
        <div class="row">
            <main id="primary" class="col-12 col-lg-8 col-xl-9 content-area">
                ...
                <?php
                    ...
                    ?>
                    <section class="bg-white border rounded shadow-sm p-3 mb-4">
                        ...
                        if (!empty($download_url)) : ?>
                            ...
                            <div class="mb-2 blockquote-footer">
                                <span> <i> <?php echo _e('If the file extension is in formats like: APKs or XAPK, please use a third-party application to install it (we recommend the SAI - Split APKs Installer app because they are easy to use).', 'mod') ?> </span> </i>
                            </div>
                    ...
                    <?php if ($other_link) : ?>
                    <section class="mb-4">
                        <h3 class="h4 font-weight-bold mb-3"><?php _e('Other available link(s)', 'mod'); ?></h3>
                ...
                <?php endif; ?>
                    ...
                    <section class="mb-4">
                        <h3 class="h4 font-weight-bold mb-3"><?php _e('Download FAQs', 'mod'); ?></h3>
                        <div id="accordion-faqs" class="accordion accordion-faqs bg-white border rounded shadow-sm p-3">
