 ...
 */
add_action( 'body_class', 'k_body_class');
function k_body_class( $classes ) {
 ...
 * Change excerpt more string
 */
function k_excerpt_length( $length ) {
    return 44;
}
add_filter( 'excerpt_length', 'k_excerpt_length' );

/**
 * Make links
 */
function k_make_links($text) {
 ...
 * Get social icons
 */
function willgroup_get_social_icons() {
    return array(
        ''            => __('Select Icon', 'willgroup'),
 ...
 */
function k_add_noindex_to_download($str) {
    global $wp_query;
...
add_filter( 'wpseo_robots', 'k_add_noindex_to_download', 10, 1 );

/**
 * Remove canonical from download
 */
function k_remove_canonical_from_download( $canonical ) {
    global $wp_query;
    if ( ! empty( $wp_query->query_vars['download'] ) ) {
        $download_slug = $wp_query->query_vars['download'];
        $download_slugs = explode('/', $download_slug);
...
add_filter( 'wpseo_canonical', 'k_remove_canonical_from_download', 99, 1 );

/**
 * Add download title
 */
function k_change_download_title($title) {
    global $wp_query;
    if ( ! empty( $wp_query->query_vars['download'] ) ) {
        $download_slug = $wp_query->query_vars['download'];
        $download_slugs = explode('/', $download_slug);
...
add_filter('wpseo_title', 'k_change_download_title');

/**
 * Add download description
 */
function k_change_download_desc($desc) {
    global $wp_query;
    if ( ! empty( $wp_query->query_vars['download'] ) ) {
        $download_slug = $wp_query->query_vars['download'];
        $download_slugs = explode('/', $download_slug);
...
add_filter('wpseo_metadesc', 'k_change_download_desc');

/**
 * Add download endpoint
 */
function k_add_download_endpoint() {
    remove_action('template_redirect', 'redirect_canonical');
    add_rewrite_endpoint( 'download', EP_ROOT );
}
add_action( 'init', 'k_add_download_endpoint' );

function k_download_template_redirect() {
...
add_action( 'template_redirect', 'k_download_template_redirect' );

/**
 * Block non-administrators from accessing the WordPress back-end.
 */
function k_block_users_backend() {
...
add_action( 'init', 'k_block_users_backend' );

/**
 * Modify main query
 */
function k_modify_main_query( $query ) {
    if ( $query->is_tag() && $query->is_main_query() && ! is_admin() ) {
        $query->set( 'post_type', array('post', 'tip') );
    }
}
add_action( 'pre_get_posts', 'k_modify_main_query' );

add_filter('wpseo_metadesc','willgroup_change_metadesc');
function willgroup_change_metadesc( $desc ) {
    if ( is_singular('post') ) {
 ...
 * Change og meta
 */
add_filter( 'wpseo_opengraph_title', 'willgroup_change_opengraph_title', 10, 1 );
function willgroup_change_opengraph_title( $title ) {
	global $wp_query;
    if ( is_singular('post') ) {
...
add_filter( 'wpseo_opengraph_desc', 'willgroup_change_opengraph_desc', 10, 1 );
function willgroup_change_opengraph_desc( $desc ) {
	global $wp_query;
    if ( is_singular('post') ) {
        if ( $short_description = get_field('short_description') ) {
            $desc = strip_tags($short_description);
...
add_filter('wpseo_frontend_presentation', 'willgroup_yoast_presentation', 30);
function willgroup_yoast_presentation($presentation) {
	global $wp_query;
    if ( is_singular('post') ) {
 ...
 */
function willgroup_change_category_canonical( $canonical ) {
	if ( is_category() ) {
...
add_filter( 'wpseo_canonical', 'willgroup_change_category_canonical' );


/**
 * Clean download cache when update or delete post
 */
function k_clean_download_cache( $post_id, $post, $update ) {
    ...
    k_delete_directory(WP_ROCKET_CACHE_ROOT_PATH . 'wp-rocket/modjuegos.es/download/' . $download_slug);
}
add_action( 'save_post', 'k_clean_download_cache', 10, 3 );

/*
 * php delete function that deals with directories recursively
 */
function k_delete_directory($dirname) {
         if (is_dir($dirname))
     ...
     while($file = readdir($dir_handle)) {
           if ($file != "." && $file != "..") {
                if (!is_dir($dirname."/".$file))
                    ...
                    k_delete_directory($dirname.'/'.$file);
           }
     }
     closedir($dir_handle);
     rmdir($dirname);
