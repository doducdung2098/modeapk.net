...
function version_checking_init() {
    ...
    if( function_exists('acf_add_options_page') ) {
        acf_add_options_page(array(
            'page_title' 	  => 'Version Checking',
            'menu_title'	  => 'Version Checking',
...
function get_playstore_info_v1($packname){
    try {
        require_once(get_template_directory() . '/vendor/autoload.php');
        $gplay = new \Nelexa\GPlay\GPlayApps('ru_RU', 'RU');
        $packages = array();
        array_push($packages, $packname);

        $apps = $gplay->getAppsInfo($packages);
        $data = array();
        foreach ( $apps as $app ) {
            $data['version'] = trim($app->getAppVersion());
            $data['title'] = $app->getName();
            $data['android'] = $app->getMinAndroidVersion();
            $data['publisher'] = $app->getDeveloperName();
            $data['whats_new'] = $app->getRecentChanges();
            break;
        }
...
function get_playstore_info_v2($packname){

    $url = 'https://play.google.com/store/apps/details?id='.$packname.'&hl=en&gl=gl';
...
function get_playstore_version_v1($packname) {
    try {
        $info = get_playstore_info_v1($packname);
...
function get_playstore_version_v2($packname){
    try {
        $response = get_playstore_info_v2($packname);
...
function get_playstore_version($packname){
    return get_playstore_version_v2($packname);
}

function get_playstore_info($packname){
//    return get_playstore_info_v1($packname);
    return get_playstore_info_v2($packname);
}

function get_combo_version($packname) {
    $url = 'https://v.apkoyun.club/version.php?package='.$packname.'&token=6a505a9a759369e697f0c56f2048adf6';
...
function update_record($post_id, $packname, $new_version){
    global $wpdb;
...
add_action('k_version_checking', 'k_version_checking');
function k_version_checking() {
    ...
    if ($query->have_posts()) :
        ...
        while ($query->have_posts()) : $query->the_post();
            ...
            if (stripos($url, $needle) !== false) :
                ...
                $new_version = trim(get_combo_version($packname));
                $version = trim(get_field('_softwareVersion'));

                if( $version && $new_version && version_compare($version, $new_version, '<') ) : // check from combo
                    update_record(get_the_ID(), $packname, $new_version);
                else : // check from playstore
                    $new_version = get_playstore_version($packname);
...
function mod_after_version_checking_page() {
    ...
    <p>
        <b>Sort by:
            <a href="<?php echo $base_url.'/wp-admin/edit.php?page=version-checking'; ?>">Times</a> /
            <a href="<?php echo $base_url.'/wp-admin/edit.php?page=version-checking&ord=post_id'; ?>">Title</a> /
            <a href="<?php echo $base_url.'/wp-admin/edit.php?page=version-checking&ord=views'; ?>">Views</a>
        </b>
    </p>
            ...
            <?php foreach ( $rows as $row ) :
                ...
                <tr >
                    <td class="first-col">
                        ...
                        <a target="_blank" href="https://apkpure.com/en/<?php echo $row->packname ?>">PURE</a> /
                    ...
                    <td>
                        ...
                        <?php else : ?>
                            <a class="button update checking_update_btn" data-new_version="<?php echo $row->new_version; ?>" data-post_id="<?php echo $row->post_id; ?>" href="javascript:void(0);">
                                Update
                            </a>
                        <?php endif; ?>
                    </td>
                    ...
                    <td class="plugins">
                        ...
                        <a class="delete checking_delete_btn" data-id="<?php echo $row->ID; ?>" href="javascript:void(0);">
